...
assert_json "$TEST_TMP/results/success.out.metrics.json" \
  '.schema_version == 3 and .model == "glm-5.3-flash" and .requested_model == "glm-5.3-flash" and
   .model_identity_source == "requested-only" and .tokens.input == 3 and .tokens.output == 1 and
...
for removed in --evaluation --preflight-only "--evaluation-manifest x"; do
  ...
  PATH="$TEST_TMP/bin:$PATH" ZAI_API_KEY="fixture-key" FAKE_CLAUDE_CASE=success \
    "$ROOT/bin/delegation-glm" run --lane scout --prompt-file "$TEST_TMP/prompt" \
      --output "$TEST_TMP/results/removed.out" --workdir "$TEST_TMP/work" $removed >/dev/null 2>&1 || rc=$?
  [ "$rc" -eq 64 ] || fail "removed flag $removed returned $rc, expected 64"
done

# check reports the adapter's roles and efforts, and nothing about qualification.
PATH="$TEST_TMP/bin:$PATH" ZAI_API_KEY="fixture-key" \
  "$ROOT/bin/delegation-glm" check --json >"$TEST_TMP/results/check.json"
assert_json "$TEST_TMP/results/check.json" \
  '.model == "glm-5.3-flash" and .adapter == "claude-zai" and .selected_backend == "claude-zai" and
   .roles == ["clerk","scout","builder","reviewer","policy-annotation"] and .efforts == ["max"] and
   (has("qualified_lanes") | not) and (has("provisional_lanes") | not)'
# an unsupported role is refused before any dispatch (78)
rc=0
PATH="$TEST_TMP/bin:$PATH" ZAI_API_KEY="fixture-key" FAKE_CLAUDE_CASE=success \
  "$ROOT/bin/delegation-glm" run --lane judgement --prompt-file "$TEST_TMP/prompt" \
    --output "$TEST_TMP/results/unsupported.out" --workdir "$TEST_TMP/work" >/dev/null 2>&1 || rc=$?
[ "$rc" -eq 64 ] || fail "unsupported role returned $rc, expected 64"
...
# Destinations that are directories are refused before dispatch.
mkdir -p "$TEST_TMP/output-dir" "$TEST_TMP/metrics-dir"
rc=0
PATH="$TEST_TMP/bin:$PATH" ZAI_API_KEY="fixture-key" FAKE_CLAUDE_CASE=success \
  "$ROOT/bin/delegation-glm" run \
    --lane scout --effort auto --backend auto \
