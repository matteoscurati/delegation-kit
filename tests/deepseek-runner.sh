...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/delegation-deepseek-test.XXXXXX")"
trap 'rm -rf -- "$TMP"' EXIT
...
[ -n "$output" ] || exit 90
if [ -n "${FAKE_DEEPSEEK_REQUEST_CAPTURE:-}" ]; then
  cp "$request" "$FAKE_DEEPSEEK_REQUEST_CAPTURE" || exit 91
fi
case "${FAKE_DEEPSEEK_CASE:-success}" in
  success)
    printf '%s\n' '{"model":"deepseek-v4-pro","choices":[{"message":{"content":"PONG"}}],"usage":{"prompt_tokens":7,"completion_tokens":3}}' >"$output"
    printf '200'
    ;;
  auth)
    printf '%s\n' '{"model":"deepseek-v4-pro","error":{"message":"SECRET_PROVIDER_RESPONSE"}}' >"$output"
    printf '401'
    ;;
  rate)
    printf '%s\n' '{"model":"deepseek-v4-pro","error":{"message":"SECRET_PROVIDER_RESPONSE"}}' >"$output"
  ...
  malformed)
    ...
    ;;
  empty)
    printf '%s\n' '{"model":"deepseek-v4-pro","choices":[{"message":{"content":""}}]}' >"$output"
    printf '200'
    ;;
  identity)
    printf '%s\n' '{"model":"deepseek-v4-pro-fallback","choices":[{"message":{"content":"PONG"}}]}' >"$output"
    printf '200'
    ;;
  transport)
...
write_manifest() {
  jq -n \
    ...
    --arg runner_sha256 "$(sha256 "$ROOT/bin/delegation-deepseek")" \
    ...
    '{schema:"delegation_policy_annotation_evaluation_v1",profile:"deepseek-v4-pro",lane:"policy-annotation",model:"deepseek-v4-pro",backend:"deepseek-api",effort:"max",prompt_sha256:$prompt_sha256,runner_source_commit:$source_commit,runner_sha256:$runner_sha256,contract_path:"evaluation/test-fixtures/contract.txt",contract_sha256:$contract_sha256,output_schema_path:"evaluation/test-fixtures/output-schema.json",output_schema_sha256:$output_schema_sha256,timeout_seconds:60,max_output_chars:1024}' \
    >"$TMP/manifest.json"
}
write_manifest
MANIFEST_SHA="$(sha256 "$TMP/manifest.json")"
jq --arg hash "$MANIFEST_SHA" '
  .profiles["deepseek-v4-pro"].lanes["policy-annotation"].evaluation_manifest_sha256 = [$hash]
' "$ROOT/config/routing-gates.json" >"$TMP/gates.json"
mv "$TMP/gates.json" "$ROOT/config/routing-gates.json"
jq --arg hash "$MANIFEST_SHA" '
  .lanes["policy-annotation"].backends["deepseek-api"].evaluation_manifest_sha256 = [$hash]
' "$ROOT/config/deepseek-v4-pro-routing.json" >"$TMP/deepseek-routing.json"
mv "$TMP/deepseek-routing.json" "$ROOT/config/deepseek-v4-pro-routing.json"
git -C "$ROOT" add config/routing-gates.json config/deepseek-v4-pro-routing.json
...
run_case() {
  ...
  PATH="$TMP/bin:$PATH" TMPDIR="$TMP/runtime" \
    DEEPSEEK_API_KEY=sk-sp-test FAKE_DEEPSEEK_CASE="$name" \
    FAKE_DEEPSEEK_REQUEST_CAPTURE="$TMP/results/$name.request.json" \
    "$ROOT/bin/delegation-deepseek" run --lane policy-annotation --effort auto \
    --backend deepseek-api --evaluation --prompt-file "$TMP/prompt" \
...
PATH="$TMP/bin:$PATH" DEEPSEEK_API_KEY=sk-sp-test \
  "$ROOT/bin/delegation-deepseek" check --json >"$TMP/check.json"
json "$TMP/check.json" \
  '.model == "deepseek-v4-pro" and .backends["deepseek-api"].available == true'
...
rc=0
PATH="/usr/bin:/bin" DEEPSEEK_API_KEY=sk-sp-test \
  "$ROOT/bin/delegation-deepseek" run --lane builder --prompt-file "$TMP/prompt" \
...
rc=0
PATH="$TMP/bin:$PATH" TMPDIR="$TMP/runtime" \
  DEEPSEEK_API_KEY=sk-sp-test FAKE_DEEPSEEK_CASE=success \
  FAKE_DEEPSEEK_REQUEST_CAPTURE="$TMP/results/builder.request.json" \
  "$ROOT/bin/delegation-deepseek" run --lane builder --allow-provisional \
...
json "$TMP/results/builder.out.metrics.json" \
  '.model == "deepseek-v4-pro" and .lane == "builder" and .effort == "max"'
json "$TMP/results/builder.out.metrics.json" 'has("evaluation_receipt") | not'
json "$TMP/results/builder.request.json" '.max_tokens == 4096 and .reasoning_effort == "max" and .thinking.type == "enabled"'
...
rc=0
PATH="$TMP/bin:$PATH" DEEPSEEK_API_KEY=sk-sp-test FAKE_DEEPSEEK_CASE=success \
  "$ROOT/bin/delegation-deepseek" run --lane builder --allow-provisional --effort high \
...
rc=0
PATH="/usr/bin:/bin" DEEPSEEK_API_KEY='' \
  "$ROOT/bin/delegation-deepseek" run --lane judgement --evaluation \
...
json "$TMP/results/success.out.metrics.json" \
  '.model == "deepseek-v4-pro" and .effort == "max" and
...
jq -e --arg manifest_sha "$MANIFEST_SHA" \
  ...
  --arg runner_sha "$(sha256 "$ROOT/bin/delegation-deepseek")" \
...
for phase in output metrics commit; do
  ...
  FAIL_MV_TARGET="$target" PATH="$TMP/mv-fail:$TMP/bin:$PATH" \
    TMPDIR="$TMP/runtime" DEEPSEEK_API_KEY=sk-sp-test FAKE_DEEPSEEK_CASE=success \
    "$ROOT/bin/delegation-deepseek" run --lane policy-annotation --effort auto \
    --backend deepseek-api --evaluation --prompt-file "$TMP/prompt" \
...
debug_run="$(find "$TMP/debug" -maxdepth 1 -type d -name 'delegation-deepseek.*' | head -1)"
...
rc=0
PATH="$TMP/bin:$PATH" DEEPSEEK_API_KEY=sk-sp-test \
  "$ROOT/bin/delegation-deepseek" run --lane policy-annotation --evaluation \
  --evaluation-manifest "$TMP/manifest.json" \
...
rc=0
PATH="$TMP/bin:$PATH" TMPDIR="$TMP/runtime" DEEPSEEK_API_KEY=sk-sp-test \
  FAKE_DEEPSEEK_CASE=success FAKE_DEEPSEEK_REQUEST_CAPTURE="$TMP/results/preflight.request.json" \
  "$ROOT/bin/delegation-deepseek" run --lane policy-annotation --effort auto \
  --backend deepseek-api --evaluation --evaluation-manifest "$TMP/manifest.json" \
...
jq -e --arg manifest_sha "$MANIFEST_SHA" \
  ...
  '(keys | sort) ==
   ...
   .profile == "deepseek-v4-pro" and .model == "deepseek-v4-pro" and
   .backend == "deepseek-api" and .effort == "max" and
...
rc=0
PATH="$TMP/bin:$PATH" DEEPSEEK_API_KEY=sk-sp-test \
  "$ROOT/bin/delegation-deepseek" run --lane policy-annotation --preflight-only \
...
rc=0
PATH="$TMP/bin:$PATH" DEEPSEEK_API_KEY=sk-sp-test \
  FAKE_DEEPSEEK_CASE=success FAKE_DEEPSEEK_REQUEST_CAPTURE="$TMP/results/preflight-missing.request.json" \
  "$ROOT/bin/delegation-deepseek" run --lane policy-annotation --evaluation \
...
rc=0
PATH="$TMP/bin:$PATH" TMPDIR="$TMP/runtime" DEEPSEEK_API_KEY=sk-sp-test \
  FAKE_DEEPSEEK_CASE=success FAKE_DEEPSEEK_REQUEST_CAPTURE="$TMP/results/preflight-900.request.json" \
  "$ROOT/bin/delegation-deepseek" run --lane policy-annotation --effort auto \
  --backend deepseek-api --evaluation --evaluation-manifest "$TMP/manifest-timeout900.json" \
  --preflight-only --prompt-file "$TMP/prompt" \
...
rc=0
PATH="$TMP/bin:$PATH" TMPDIR="$TMP/runtime" DEEPSEEK_API_KEY=sk-sp-test \
  FAKE_DEEPSEEK_CASE=success FAKE_DEEPSEEK_REQUEST_CAPTURE="$TMP/results/preflight-901.request.json" \
  "$ROOT/bin/delegation-deepseek" run --lane policy-annotation --effort auto \
  --backend deepseek-api --evaluation --evaluation-manifest "$TMP/manifest-timeout901.json" \
  --preflight-only --prompt-file "$TMP/prompt" \
...
jq '.profiles["deepseek-v4-pro"].lanes["policy-annotation"].status = "qualified" |
    .profiles["deepseek-v4-pro"].lanes["policy-annotation"].selection = "explicit-only" |
    .profiles["deepseek-v4-pro"].lanes["policy-annotation"].evaluation_manifest_sha256 = []' \
  "$ROOT/config/routing-gates.json" >"$TMP/gates.json"
mv "$TMP/gates.json" "$ROOT/config/routing-gates.json"
jq '.qualified_lanes = ["policy-annotation"] |
    .lanes["policy-annotation"].backends["deepseek-api"].status = "qualified" |
    .lanes["policy-annotation"].backends["deepseek-api"].selection = "explicit-only" |
    .lanes["policy-annotation"].backends["deepseek-api"].qualified = true |
    del(.lanes["policy-annotation"].backends["deepseek-api"].evaluation_manifest_sha256)' \
  "$ROOT/config/deepseek-v4-pro-routing.json" >"$TMP/deepseek-routing.json"
mv "$TMP/deepseek-routing.json" "$ROOT/config/deepseek-v4-pro-routing.json"
git -C "$ROOT" add config/routing-gates.json config/deepseek-v4-pro-routing.json
...
PATH="$TMP/bin:$PATH" TMPDIR="$TMP/runtime" \
  DEEPSEEK_API_KEY=sk-sp-test FAKE_DEEPSEEK_CASE=success \
  FAKE_DEEPSEEK_REQUEST_CAPTURE="$TMP/results/ordinary.request.json" \
  "$ROOT/bin/delegation-deepseek" run --lane policy-annotation --effort auto \
  --backend deepseek-api --prompt-file "$TMP/prompt" \
...
json "$TMP/results/ordinary.out.metrics.json" \
  '.model == "deepseek-v4-pro" and .tokens.input == 7 and .tokens.output == 3'
...
[ -z "$(find "$TMP/runtime" -mindepth 1 -maxdepth 1 -name 'delegation-deepseek.*' -print)" ] \
  || fail 'temporary directories not cleaned'
printf 'DeepSeek runner diagnostics tests passed.\n'
