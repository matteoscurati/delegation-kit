...
DelegationKit exposes optional lanes and profiles, but their presence grants no
standing permission to invoke another agent. Do not call Agent, Workflow,
`codex exec`, `claude -p`, or any `delegation-* run` command unless the current
user request explicitly asks for delegation.
