...
| GLM-5.3 | `clerk`, `scout`, `builder` | The exact high/max comparison ran serially with three no-retry attempts per lane on one frozen runner. Both efforts scored 1.0 in all nine attempts and every builder checker passed; the preregistered efficiency rule selected high (372s, $0.334234) over max (505s, $0.719138). Clerk and scout are qualified explicit-only, builder is provisional explicit-only, max is blocked as not selected, and GLM-5.2 is superseded. Against historical GLM-5.2/high, selected high preserved quality while reducing total elapsed time from 976s to 372s and provider-reported cost from $1.159437 to $0.334234. |
