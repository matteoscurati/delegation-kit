...
| GLM-5.3 | `clerk`, `scout`, `builder` | The exact high/max comparison ran serially with three no-retry attempts per lane. Max scored 1.0 in all nine attempts and every builder checker passed; clerk and scout are qualified explicit-only and builder is provisional explicit-only. Against historical GLM-5.2/high it preserved quality while reducing total elapsed time from 976s to 487s and provider-reported cost from $1.159437 to $0.622429. High remains blocked after one builder attempt claimed the change without modifying the fixture; GLM-5.2 is superseded. |
