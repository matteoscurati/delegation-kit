...
| GLM-5.3-Flash/max | none (staged candidates) | The corrected v2 task pack passed 9/9 no-retry operational checks at score 1.0 and every builder checker passed. Strict identity is `VOID` because retained evidence did not record separately surfaced effective content identity or complete model usage, so no lane is promoted; this does not establish what the provider originally exposed. The v1 sandbox attempt is a separate terminal pre-provider `VOID`. |
