...
(cost / intelligence / taste per model) lives in `model-routing.md` at the kit
root — read it for the numbers; this skill is the decision procedure.
...
If a GPT/Codex model earns a lane in your table, drive it from Claude via the
Codex CLI: `codex exec -s read-only "<self-contained prompt>" </dev/null`
(`-s workspace-write` to let it edit). Pin a cheap model with an ephemeral
profile: `codex exec --ephemeral -p <profile> "<prompt>" </dev/null`. Use it for a
cheap high-volume lane or an independent second-opinion review from a different
model family.
